...
ESConnector.prototype.setupIndex = function (callback) {
    var self = this;
    var db = self.db;

    log('ESConnector.prototype.setupIndex', 'index', self.searchIndex);
    log('ESConnector.prototype.setupIndex', 'settings', self.searchIndexSettings);

    self.db.indices.exists({ index: self.searchIndex })
      .then(function(exists) {
        if (!exists) {
          return self.db.indices.create({
      ...
      .catch(function(err){
          log('ESConnector.prototype.setupIndex', 'failed');
          callback(err);
      });
};
...
ESConnector.prototype.addDefaults = function (modelName) {
    ...
    filter.type = this.searchType || modelName;
    ...
    if (criteria) {
        ...
        if(criteria.order) {
            ...
            if (idName === 'id' && modelProperties.id.generated) {
                ...
                console.log('will sort on _uid by default when IDs are auto-generated by elasticsearch');
                filter.body.sort = ['_uid'];
            } else {
                console.log('will sort on loopback specified IDs');
    ...
    if (_.keys(where).length===0) {
        ...
        _.forEach(where, function(value, key) {
            ...
            if (spec) {
                ...
                if (spec === 'neq') {
                    var match = {match:{}};
                    match.match[key] = cond;
                    body.query.bool.must_not.push(match);
                }
                // TODO: like, nlike
            }
            else {
...
ESConnector.prototype.automigrate = function (models, cb) {
    ...
    if (self.db) {
        ...
        self.db.indices.delete({index: self.searchIndex})
            .then(function() {
              self.setupIndex(function(err) {
                  if (err) {
                      cb(err);
