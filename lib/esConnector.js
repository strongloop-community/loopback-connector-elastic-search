  ...
  if (filter && filter.suggests) { // TODO: remove HACK!!!
    self.db.suggest(
      ...
      function(body) {
        ...
        //include total result on response
        var _data = [];
        var total = body.hits.total;

        if (body.hits) {
          body.hits.hits.forEach(function(item) {
            _data.push(self.dataSourceToModel(model, item, idName));
          });
        }
...
ESConnector.prototype.destroyAll = function destroyAll(modelName, whereClause, cb) {
  ...
  self.db.deleteByQuery(_.defaults({
    body: body
  }, defaults)).then(
    function(response) {
      cb(null, response);
