...
var generateExtension = require('./dao-extension');
...
var ESConnector = function (settings, dataSource) {
  ...
  // NOTE: for more information, see loopback-datasource-juggler:lib/datasource.js:128
  var defaultDAO = dataSource.constructor.DataAccessObject;
  var DAOextensions = generateExtension(this);
  var extendedDAO = _.assign(defaultDAO, DAOextensions);
  this.DataAccessObject = extendedDAO;
};
 ...
 *    > When writing a query without an order specified, the author should not assume any reliable
 * order.
 *    > So if we’re not assuming any order, there is not a compelling reason to potentially slow
 * down
...
ESConnector.prototype.updateOrCreate = function updateOrCreate(modelName, data, callback) {
  ...
  self.db.update(_.defaults({
    ...
    function (response) {
       ...
       *       and back is to support databases that can add default values to undefined
       * properties,
       ...
       *       A: [Victor Law]
       *          - Most connectors don't fetch the inserted/updated data and hope the data stored
       * into DB will be the same as the data sent to DB for create/update.
...
/**
 * direct access to the Elasticsearch bulk API
 * https://www.elastic.co/guide/en/elasticsearch/reference/current/docs-bulk.html
 *
 * @param modelName {String} - required, model that is accociated with this connector
 * @param params {Object} - required, same as the native bulk function uses
 * @param done {Function} - allows async execution
 * @throws {Error} - if argument rules are violated
 * @returns {Promise} OR {undefined} if callback (done) is provided

 */
ESConnector.prototype.bulk = function bulk(modelName, params, done){
  var self = this;
  if (self.debug) {
    log('ESConnector.prototype.bulk', modelName, params);
  }

  if (typeof params !== 'object' || params === null) {
    throw new Error('invalid argument: params must be defined');
  }
  if (Array.isArray(params.body) === false) {
    throw new Error('invalid argument: params.body must be an array');
  }
  if (params.body.length <= 0) {
    var res = {took: 0, errors: false, items: []};
  ...
  return self.db
    ...
    .then(
      function(res){
        log('ESConnector.prototype.bulk', 'response', res);
        if (typeof done === 'function') {
          log('ESConnector.prototype.bulk', 'will invoke callback with full response:', res);
          done(null, res);
        } else {
          return Promise.resolve(res);
        }
...
/**
 * ES bulk insert + index
 * @param modelName {String} - required, model that is associated with this connector
 * @param dataList {Array} - required, list of documents/models, that gets inserted and indexed
 * @param params {Object} - passed, through to the bulk API
 * @param done {Function} - required, allows async execution
 * @returns {Promise} OR {undefined} if callback (done) is provided
 * @value: {[String]} - return value is an array of object IDs
 * @throws {Error} - if argument rules are violated
 *
 * NOTE: if params has a field called `body`, it will be ignored. Instead dataList is used
 */
ESConnector.prototype.bulkCreate = function bulkCreate(modelName, dataList, params, done) {
  var self = this;
  var functionName = 'bulkCreate';
  if (self.debug) {
    log('ESConnector.prototype.' + functionName, modelName, dataList, params);
  }

  if (Array.isArray(dataList) === false) {
    var typeError = new Error('invalid type for dataList');
    if (typeof done === 'function') {
  ...
  // NOTE: incl. the names of type and index, if configured
  var defaultParams = self.addDefaults(modelName, functionName);
  var bulkParams = _.assign({}, defaultParams, params);

  bulkParams.body = dataList.reduce(function(dataBulk, data){
    var idValue = self.getIdValue(modelName, data);

    var clonedData = _.assign({}, data);

    delete clonedData[idName];
    var action = {};
    action[actionName] = { _id: idValue };
    dataBulk.push(action, clonedData);

    return dataBulk;
  }, [ /* dataBulk */ ]);

  return self
    .bulk(modelName, bulkParams)
    .then(
      function(res){
        log('ESConnector.prototype.bulkCreate', 'successful executed in: ' + res.took + 'ms');
        
        var result = res.items.map(function(item){
          return item[actionName]._id;
        });
 ...
 * ES bulk update
 * @param modelName {String} - required, model that is associated with this connector
 * @param dataList {Array} - required, list of documents/models containing its ID and the changed
 * @param params {Object} - passed, through to the bulk API
 * @param done {Function} - required, allows async execution
 * @returns {Promise} OR {undefined} if callback (done) is provided
 * @value: {Number} - return the amount of affected items
 * @throws {Error} - if argument rules are violated
 *
 * NOTE: if params has a field called `body`, it will be ignored. Instead dataList is used
 */
ESConnector.prototype.bulkUpdate = function bulkUpdate(modelName, dataList, params, done){
  var self = this;
  var functionName = 'bulkUpdate';
  if (self.debug) {
    log('ESConnector.prototype.' + functionName, modelName, dataList, params);
  }
  ...
  bulkParams.body = dataList.reduce(function(dataBulk, data){
    ...
    var action = {};
    action[actionName] = { _id: idValue };
    dataBulk.push(action, { doc: clonedData });
  ...
  return self
    ...
    .then(
      function(res){
        log('ESConnector.prototype.bulkUpdate', 'successful executed in: ' + res.took + 'ms');

        var affectedAmount = res.items.reduce(function(amount, item){
          if(item[actionName].result === 'updated'){
            amount += 1;
          }
          return amount;
        }, 0);
        ...
        if (typeof done === 'function') {
          done(null, affectedAmount);
        } else {
          return Promise.resolve(affectedAmount);
        }
      },
      function(err){
        console.trace(err.message);
 ...
 * ES bulk delete
 * @param modelName {String} - required, model that is associated with this connector
 * @param dataList {Array} - required, list of documents/models containing its ID
 * @param params {Object} - passed, through to the bulk API
 * @param done {Function} - required, allows async execution
 * @returns {Promise} OR {undefined} if callback (done) is provided
 * @value: {[Boolean]} - return an array, stating if the corresponding item has been deleted
 * @throws {Error} - if argument rules are violated
 *
 * NOTE: if params has a field called `body`, it will be ignored. Instead dataList is used
 */
ESConnector.prototype.bulkDestroy = function bulkDelete(modelName, dataList, params, done){
  var self = this;
  var functionName = 'bulkDelete';
  if (self.debug) {
    log('ESConnector.prototype.' + functionName, modelName, dataList, params);
  }
  ...
  bulkParams.body = dataList.map(function(data){
    var idValue = self.getIdValue(modelName, data);
  ...
  return self
    ...
    .then(
      function(res){
        log('ESConnector.prototype.bulkDestroy', 'successful executed in: ' + res.took + 'ms');

        var report = res.items.map(function(item){
          return item[actionName].found && item[actionName].result === 'deleted';
        });
 ...
 *
 * @param {String[]} [models] A model name or an array of model names. If not present, apply to all
 *     models
